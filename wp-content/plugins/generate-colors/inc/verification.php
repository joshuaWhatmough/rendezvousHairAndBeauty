...
if ( ! function_exists( 'generate_verify_email' ) ) :
	...
	function generate_verify_email()
	{
		...
		<?php if ( false == $email ) : ?>
			<p>
				<?php _e('Want updates served directly to your Dashboard? Activate the email you purchased your add-ons with here.','generate'); ?>
			</p>
		<?php endif; ?>
		<form method="post" action="options.php">
					...
					<?php if ( true == $email ) : ?>
						<div class="add-on activated gp-clear grid-container" style="border-left:5px solid #2ea2cc !important">
							<div class="addon-name" style="position: relative;">
								<input placeholder="<?php _e( 'Email','generate' );?>" id="generate_customer_email" name="generate_customer_email" type="email" class="regular-text" value="<?php echo $generate_customer_email; ?>" style="display:none"  />
							...
							<div class="addon-action" style="text-align:right;">
								...
								<input type="submit" class="deactivate-button" name="generate_customer_email_deactivate" value="<?php _e('Deactivate Email','generate');?>"/>
							</div>
						</div>
					<?php else : ?>
						<div class="add-on gp-clear grid-container" style="background-color:#fafafa;">
							<div class="addon-name" style="position:relative;">
								...
								<input placeholder="<?php _e( 'Email','generate' );?>" id="generate_customer_email" name="generate_customer_email" type="email" class="regular-text" value="<?php echo $generate_customer_email; ?>" style="width:100%" />
							</div>
							<?php if ( '' !== $generate_customer_email ) : ?>
								<div class="addon-action" style="text-align:right;">
									<?php wp_nonce_field( 'generate_customer_email_nonce', 'generate_customer_email_nonce' ); ?>
									<input type="submit" class="activate-button" name="generate_customer_email_activate" value="<?php _e('Activate Email','generate');?>" style="line-height:28px"/>
								</div>
							<?php endif; ?>
						</div>
					<?php endif; ?>
			...
			<?php if ( true == $email ) : ?>
				...
				<input type="submit" class="button" name="generate_activate_all" value="<?php _e('Enable All Updates','generate');?>" style="margin:10px 0;"/>
		...
		if ( $key == 'valid' && $email_status == 'valid' ) { ?>
			<div class="add-on activated gp-clear grid-container" style="border-left:5px solid #2ea2cc !important">
				...
				<div class="addon-action" style="text-align:right;">
					...
					<input type="submit" name="generate_deactivate_<?php echo $id;?>" value="<?php _e('Disable Updates','generate');?>"/>
				</div>
			</div>
		<?php } elseif ( $key !== 'valid' && $email_status == 'valid' ) { ?>
			<div class="add-on gp-clear grid-container">
				...
				<div class="addon-action" style="text-align:right;">
					...
					<input type="submit" name="generate_activate_<?php echo $id;?>" value="<?php _e('Enable Updates','generate');?>"/>
				</div>
			</div>
		<?php }
...
function generate_license_errors() 
{
	...
	if ( isset( $_GET['generate-message'] ) && 'email_failed' == $_GET['generate-message'] ) {
		 add_settings_error( 'generate-license-notices', 'email_failed', __( 'Email activation failed. This email does not exist in our records.', 'generate' ), 'error' );
	}
	
	if ( isset( $_GET['generate-message'] ) && 'email_verified' == $_GET['generate-message'] ) {
		 add_settings_error( 'generate-license-notices', 'email_verified', __( 'Email activated.', 'generate' ), 'updated' );
	}
	
	if ( isset( $_GET['generate-message'] ) && 'email_deactivated' == $_GET['generate-message'] ) {
		 add_settings_error( 'generate-license-notices', 'email_deactivated', __( 'Email deactivated.', 'generate' ), 'updated' );
	}
	...
	if ( isset( $_GET['generate-message'] ) && 'licenses_activated' == $_GET['generate-message'] ) {
		 add_settings_error( 'generate-license-notices', 'licenses_activated', __( 'Licenses activated.', 'generate' ), 'updated' );
	}
	
	if ( isset( $_GET['generate-message'] ) && 'deactivation_failed' == $_GET['generate-message'] ) {
		 add_settings_error( 'generate-license-notices', 'deactivation_failed', __( 'Deactivation failed.', 'generate' ), 'error' );
	}
