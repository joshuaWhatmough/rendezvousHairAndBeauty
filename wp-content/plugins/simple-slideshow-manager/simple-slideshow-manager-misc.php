...
{
$acx_slideshow_misc_hidden = $_POST['acx_slideshow_misc_hidden'];
}
else
{
$acx_slideshow_misc_hidden = '';
}
if($acx_slideshow_misc_hidden =="y")
{   
	//form data send
	if(!isset($_POST['acx_slideshow_misc_config'])) die("<br><br>Unknown Error Occurred, Try Again... <a href=''>Click Here</a>");
	if(!wp_verify_nonce($_POST['acx_slideshow_misc_config'],'acx_slideshow_misc_config')) die("<br><br>Unknown Error Occurred, Try Again... <a href=''>Click Here</a>");
	if(!current_user_can('manage_options')) die("<br><br>Sorry, You have no permission to do this action...</a>");
	
	$acx_slideshow_misc_acx_service_banners = sanitize_text_field($_POST['acx_slideshow_misc_acx_service_banners']);
	update_option('acx_slideshow_misc_acx_service_banners', $acx_slideshow_misc_acx_service_banners);
	$acx_slideshow_misc_hide_advert = sanitize_text_field($_POST['acx_slideshow_misc_hide_advert']);
	update_option('acx_slideshow_misc_hide_advert', $acx_slideshow_misc_hide_advert);
	$acx_slideshow_misc_user_level = sanitize_text_field($_POST['acx_slideshow_misc_user_level']);
...
<p class="widefat" style="padding:8px;width:99%;margin-top:8px;">	<?php _e('Who can Manage This:','simple-slideshow-manager' ); ?>
<select name="acx_slideshow_misc_user_level">
<option value="manage_options"<?php if ($acx_slideshow_misc_user_level == "manage_options"||$acx_slideshow_misc_user_level=="") { echo 'selected="selected"'; } ?>><?php _e('Administrator','simple-slideshow-manager'); ?> </option>
<option value="delete_others_pages"<?php if ($acx_slideshow_misc_user_level == "delete_others_pages") { echo 'selected="selected"'; } ?>><?php _e('Editor','simple-slideshow-manager'); ?> </option>
<option value="delete_published_posts"<?php if ($acx_slideshow_misc_user_level == "delete_published_posts") { echo 'selected="selected"'; } ?>><?php _e('Author','simple-slideshow-manager'); ?> </option>
</select>
...
<input name="acx_slideshow_misc_config" type="hidden" value="<?php echo wp_create_nonce('acx_slideshow_misc_config'); ?>" />
<p class="submit">
